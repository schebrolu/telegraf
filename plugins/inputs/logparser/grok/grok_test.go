...
func TestBuiltinInfluxdbHttpd(t *testing.T) {
	...
	assert.Equal(t,
		...
		m.Fields())
	assert.Equal(t, map[string]string{"http_method": "POST"}, m.Tags())
	...
	assert.Equal(t,
		...
		m.Fields())
	assert.Equal(t, map[string]string{"http_method": "GET"}, m.Tags())
}
